...
def main():
    ...
    df_traj.sort_values(by=["pse_id", "time_id"], inplace=True)
    ...
    cell_traj = np.array(list(df_traj.groupby("pse_id")["reg_id"].apply(np.array)))
    ...
    lat_lng_traj = np.array(
        list(df_traj.groupby("pse_id").apply(lambda x: np.array(x[["y(center)", "x(center)"]])))
        )
